...
import { formatCurrencyFull, formatDate, formatDateLong } from '../utils/formatters'
...
export default function CreditCardDetail({ account, onBack }: CreditCardDetailProps) {
  const { deleteCreditCardAccount, deleteCreditCardEntry } = useFinance()
  const [showUpload, setShowUpload] = useState(false)
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false)

  const sorted = [...account.entries].sort((a, b) =>
    a.statementEndDate.localeCompare(b.statementEndDate),
  )
  const latest = sorted[sorted.length - 1]

  const chartData = sorted.map((e) => ({ date: e.statementEndDate, value: e.balance }))
    ...
    <div className="p-4 md:p-8 space-y-6 md:space-y-8">
      ...
      {latest && (
        <div className="bg-[#12151f] border border-[#1e2235] rounded-2xl p-6">
          <p className="text-xs text-gray-500 mb-1">Latest Balance</p>
          <p className="text-3xl md:text-4xl font-bold text-white">
            {formatCurrencyFull(latest.balance)}
          </p>
          <p className="text-xs text-gray-600 mt-1">{formatDateLong(latest.statementEndDate)}</p>
        </div>
      )}
      ...
      {sorted.length > 1 && (
        <div className="bg-[#12151f] border border-[#1e2235] rounded-2xl p-6">
          <h2 className="text-sm font-semibold text-gray-300 mb-6">Balance Over Time</h2>
        ...
        {sorted.length === 0 ? (
          ...
          <div className="space-y-2">
            {[...sorted].reverse().map((entry, idx, arr) => {
              ...
              const pct = prev ? (change! / prev.balance) * 100 : null
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white">
                      {formatDateLong(entry.statementEndDate)}
                    </p>
                  ...
                  <button
                    onClick={() => deleteCreditCardEntry(account.id, entry.id)}
      ...
      {showUpload && (
        <CreditCardUploadModal account={account} onClose={() => setShowUpload(false)} />
      )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#12151f] border border-[#1e2235] rounded-2xl p-6 w-full max-w-sm">
            <h3 className="text-base font-semibold text-white mb-2">Delete Card?</h3>
            <p className="text-sm text-gray-400 mb-6">
              This will permanently delete{' '}
              <strong className="text-white">{account.name}</strong> and all its history.
            </p>
